...
| `deploy.sh` | Выкладка конфигуратора на сервер одной командой (запускать локально) |
| `remote-unpack.sh` | Серверная половина выкладки: распаковать и подставить адрес API |
| `watchdog.sh` | Присмотр: проверяет сайт и API, поднимает упавшее |
...
Уезжает **последний коммит**, а не рабочая копия — если есть незакоммиченные правки, скрипт про них
предупредит, но отправит именно закоммиченное. Адрес API и ключ подставляются уже на сервере, из
`/srv/khrom/.site-env`: в репозитории `supabaseConfig.js` смотрит на облачный Supabase, и трогать
его там нельзя, пока GitHub Pages раздаёт с него старый сайт.

## Присмотр

`watchdog.sh` крутится в cron каждые 5 минут: проверяет сайт и API, при неудаче перезапускает
упавшее и пишет в `/var/log/khrom-watchdog.log`. Когда всё хорошо — молчит, чтобы журнал не зарастал.

**Чего он не может — сообщить, что сервер лёг целиком:** он сам на этом сервере. Для такого нужен
внешний монитор (UptimeRobot и подобные, бесплатного тарифа хватает): адрес `https://mebel.khrom-in.ru`,
уведомление на почту. Заводится за пять минут и требует вашей учётной записи, поэтому — за вами.

## Смотреть данные глазами (Supabase Studio)

Наружу Studio не открыта намеренно: внутри неё полный доступ к базе, и городить ещё один поддомен с
паролем ради редких заглядываний незачем. Доступ через SSH-туннель — с рабочей машины:

```bash
ssh -L 3000:127.0.0.1:3000 deploy@135.106.181.182
```

Пока команда висит, открывайте `http://localhost:3000` в браузере. Закрыли окно — доступа снова нет
ни у кого, включая тех, кто найдёт наш IP.
