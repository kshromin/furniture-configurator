...
# недостающие поля ПАЧКОЙ (производитель, тип, толщина, цена, поверхности, кромка, макс. размер) —
...
def main():
    ...
    max_l = ask_num('Макс. длина детали, мм (Enter — без ограничения)', '', allow_empty=True)
    max_w = ask_num('Макс. ширина детали, мм (Enter — без ограничения)', '', allow_empty=True)
