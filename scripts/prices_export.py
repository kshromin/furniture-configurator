...
#  - листы «только цены» (Направляющие, Фурнитура) — менять можно только цену, новые строки
#    запрещены (размерная сетка направляющих фиксирована);
#  - листы с ассортиментом (ЛДСП, Профили, Цвета профилей, Наполнение дверей, Сетчатые полки,
#    Корзины, Услуги) — новая строка внизу без служебного id/ключа = новая позиция;
...
METAL_COLORS = {'white': 'Белый', 'silver': 'Серебро', 'black': 'Чёрный'}  # сетки/корзины
...
HELP_TEXT = [
    ...
    '5. Листы только с ценами (Направляющие, Фурнитура и разное): новые строки добавлять',
    '   нельзя — только менять цены. Серые строки-заголовки внутри листа просто разделяют',
    '   блоки с разными единицами измерения.',
    '   «Сетчатые полки» и «Корзины» расширяемы: новая строка без служебного ключа = новый',
    '   размер в ассортименте (цвет — Белый/Серебро/Чёрный).',
...
def main():
    ...
    # Сетчатые полки — размерная сетка теперь из каталога: новая строка = новый размер/цвет
    ws = sheet('Сетчатые полки',
               ['Название', 'Глубина, мм', 'Цвет', 'Цена ₽/пог.м', '_key'],
               hidden_cols=(5,), price_cols=(4,), widths=(22, 12, 12, 14, 14))
    for m in data['meshShelf']:
        add_row(ws, [m['name'], m['depth'], METAL_COLORS.get(m['color'], m['color']),
                     m['pricePerM'], f"{m['depth']}:{m['color']}"])

    # Корзины — размерная сетка теперь из каталога: новая строка = новый размер/цвет
    ws = sheet('Корзины',
               ['Ширина', 'Глубина', 'Высота', 'Цвет', 'Цена ₽', '_key'],
               hidden_cols=(6,), price_cols=(5,), widths=(10, 10, 10, 12, 12, 18))
    for b in data['basket']:
        add_row(ws, [b['width'], b['depth'], b['height'], METAL_COLORS.get(b['color'], b['color']),
                     b['price'], f"{b['width']}:{b['depth']}:{b['height']}:{b['color']}"])
