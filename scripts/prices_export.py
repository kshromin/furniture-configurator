...
HELP_TEXT = [
    ...
    '   На листах ЛДСП «Производитель» должен точно совпадать с уже существующим.',
...
def main():
    ...
    ws = sheet('Профили купе',
               ['Название', 'Вертикаль ₽/пог.м', 'Горизонт. верх ₽/пог.м', 'Горизонт. низ ₽/пог.м', '_id'],
               hidden_cols=(5,), price_cols=(2, 3, 4), widths=(20, 18, 20, 20, 10))
    for p in data['slidingDoor']['profiles']:
        add_row(ws, [p['name'], p['vertPerM'], p['horizTopPerM'], p['horizBottomPerM'], p['id']])

    # Цвета профилей
    ws = sheet('Цвета профилей',
               ['Название', 'Цвет (hex)', 'Множитель цены', '_id'],
               hidden_cols=(4,), price_cols=(3,), widths=(20, 12, 16, 10))
    for c in data['slidingDoor']['colors']:
        add_row(ws, [c['name'], c['hex'], c['priceMul'], c['id']])
