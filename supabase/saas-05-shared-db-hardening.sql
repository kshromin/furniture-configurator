...
-- Разграничение прав в ОБЩЕЙ базе: конфигуратор мебели и чат-приложение ChatConfig сидят на ОДНОМ
-- проекте Supabase (csntzolfgjlcxrsozcqb). Обе апки ходят ОДНИМ anon-ключом и ролью `authenticated`,
-- поэтому база их НЕ различает — «стены по приложению» на общем проекте технически не существует.
-- Настоящая изоляция = РАЗНЫЕ проекты Supabase (см. SUPABASE-SETUP.md, раздел про общую базу).
-- Здесь — практический минимум, который реально помогает на общем проекте.
--
