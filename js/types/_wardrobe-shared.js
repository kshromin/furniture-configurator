...
export const STIFFENER_HEIGHT = 400;
...
const MESH_WIRE_STEP = 15; // шаг частых прутков вдоль ШИРИНЫ (сами прутки идут вдоль глубины)
...
// телескопический рельс, крепится саморезами к самой верхней полке (не к задней жёсткости —
// крепления отнесены от задней стенки на VALET_MOUNT_OFFSET, чтобы не налезать на неё),
// П-образный пруток (петля) тянется вдоль глубины. Показываем в выдвинутом положении.
// Размер (заявленная длина) ограничен реальной доступной глубиной за вычетом этого отступа.
...
const VALET_MOUNT_OFFSET = 120; // отступ крепления от задней стенки

export function availableValetLengths(depth) {
  const innerDepth = depth - DOOR_DEPTH_ZONE;
  return VALET_LENGTHS.filter(v => v <= innerDepth - VALET_MOUNT_OFFSET);
}
...
export function buildWardrobeBox() {
  ...
  // Глубина ящика не может физически не влезать — если сжали короб (глубину изделия), урезаем
  // сохранённое значение до нового максимума, а не молча уменьшаем только геометрию: иначе
  // в поле ввода останется устаревшее число, не совпадающее с тем, что реально построено.
  {
    const maxDD = maxDrawerDepth(depth);
    sections.forEach(sec => { if (sec.drawerDepth > maxDD) sec.drawerDepth = maxDD; });
  }
  // Глубина сетчатой полки — фиксированный выбор (300/400/500), а не свободное число: если
  // выбранное значение перестало влезать (сжали короб), откатываемся на макс. из доступных.
  {
    sections.forEach(sec => {
      const avail = availableMeshDepths(depth);
      if (!avail.includes(sec.meshDepth)) sec.meshDepth = avail.length ? avail[avail.length - 1] : MESH_DEPTHS[0];
    });
  }
  // Аналогично для торцевого вешала — размер тоже фиксированный выбор, ограниченный глубиной.
  {
    sections.forEach(sec => {
      const avail = availableValetLengths(depth);
      if (!avail.includes(sec.valetLength)) sec.valetLength = avail.length ? avail[avail.length - 1] : VALET_LENGTHS[0];
    });
  }
  ...
  // разрыва. Силовых прутков 4 (вдоль ширины): задний — ровно на заднем крае (им заканчивается
  // полка), один ближе к переду, и два вплотную друг к другу у самого переднего края — второй
  // из них приподнят (загиб) на 35мм для удержания вещей.
  function addMeshShelf(cx, y, sw, meshDepth, colorKey) {
    ...
    // Задний пруток — ровно на заднем крае (им заканчивается полка), не отступя вглубь.
    const crossZs = [backZ, backZ + meshDepth * 0.65, frontZ - MESH_WIRE * 3, frontZ - MESH_WIRE];
    crossZs.forEach(wz => addRodBar(sw, crossR, cx, y, wz, 'x', colorKey));

    // Загиб переднего края — приподнят над плоскостью полки, для удержания вещей.
    addRodBar(sw, crossR, cx, y + MESH_LIP_HEIGHT / 2, frontZ, 'x', colorKey);
  }

  // Торцевое вешало — крепится к низу верхней полки 4 саморезами (2 спереди, 2 сзади — не к
  // задней жёсткости, крепления отнесены вперёд). Рельс идёт вдоль глубины у самой полки, а сама
  // штанга-вешалка расположена ВЕРТИКАЛЬНО ниже (петля опускается вниз на переднем крае).
  ...
  function addValet(cx, topShelfWorldY, valetLength) {
    ...
    const backZ = -depth / 2 + VALET_MOUNT_OFFSET;
    const frontZ = backZ + valetLength;
    const half = VALET_LOOP_WIDTH / 2;
    const dropH = 120; // на сколько штанга-вешалка опущена вниз от рельса

    addPanel(30, 15, 20, 0xb0b0b4, [cx - half, shelfBottomY - 7, backZ]);  // кронштейн левый задний
    addPanel(30, 15, 20, 0xb0b0b4, [cx + half, shelfBottomY - 7, backZ]);  // кронштейн правый задний
    addPanel(30, 15, 20, 0xb0b0b4, [cx - half, shelfBottomY - 7, frontZ]); // кронштейн левый передний
    addPanel(30, 15, 20, 0xb0b0b4, [cx + half, shelfBottomY - 7, frontZ]); // кронштейн правый передний
    ...
    addValetRod(valetLength, cx - half, railY, backZ + valetLength / 2, 'z'); // левый рельс (у полки)
    addValetRod(valetLength, cx + half, railY, backZ + valetLength / 2, 'z'); // правый рельс (у полки)
    addValetRod(dropH, cx - half, railY - dropH / 2, frontZ, 'y');            // левая вертикальная стойка
    addValetRod(dropH, cx + half, railY - dropH / 2, frontZ, 'y');            // правая вертикальная стойка
    addValetRod(VALET_LOOP_WIDTH, cx, railY - dropH, frontZ, 'x');            // штанга-вешалка (внизу)
  }
