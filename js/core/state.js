...
// Цвет материала, у которого не задан ни hex, ни текстура. Раньше здесь был коричневый «под
// орех» как индикатор «текстуры нет» — но он читается как НАСТОЯЩИЙ цвет материала и путает
// (7.08: серый «Пыльный» строился коричневым). Нейтральный светло-серый честнее: видно, что
// цвет не задан, и ни на один реальный декор это не похоже. Обычный путь — цвет считается сам
// из картинки при заливке текстур (scripts/textures_import.py).
const TEXTURE_PLACEHOLDER_COLOR = '#d6d3ce';
...
export const state = {
  ...
  // Встройка перегородок поимённо (задание «каждая стойка сама по себе»): dividerEmbed[i] у i-й
  // перегородки основного ряда; если ключа нет — берётся общий embed.dividers. Индекс = номер
