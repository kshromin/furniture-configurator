...
## 13. Лёгкий список + превью для вкладки «Проекты» (сессия 37)

Список на вкладке «Проекты» раньше грузил `select('*')` — то есть колонку `items` (jsonb) со
ВСЕМИ прорисовками комплекта, вместе с полным 3D-снапшотом внутри каждой. Для одного только
списка (название/клиент/дата/превью) это лишний вес, растущий с числом и сложностью прорисовок.
Список теперь запрашивает только лёгкие колонки, включая новые:

- `item_count` — число прорисовок в комплекте, дублирует `(items).length`, но отдельной
  колонкой (та же идея, что уже была у `total` — посчитано один раз при сохранении, а не
  каждый раз из `items`).
- `thumbnail` — маленькая (≈160×120) превьюшка текущего вида 3D-модели в момент сохранения
  (`renderer.domElement`, см. `captureThumbnail()` в `js/core/order.js`), base64 JPEG прямо в
  колонке — при таком разрешении это единицы КБ, отдельный storage-бакет не нужен.

Полный `items` (со снапшотами) по-прежнему грузится, но только когда открываешь КОНКРЕТНЫЙ
проект (`openProject` в `js/core/order.js`) — не для всего списка сразу.

Выполнить в **SQL Editor**:

```sql
alter table public.projects add column if not exists item_count int not null default 0;
alter table public.projects add column if not exists thumbnail text;
```

**Важно:** пока эта миграция не выполнена, сохранение проекта/заказа будет падать с ошибкой
(колонки `item_count`/`thumbnail` не существуют) — выполнить ДО того, как это обновление сайта
попадёт на прод.

---

## Общая база с чат-приложением ChatConfig (разграничение прав)

**Факт:** конфигуратор и чат-приложение ChatConfig (`CRM Chat/chat-app`) сидят на ОДНОМ проекте
Supabase (`csntzolfgjlcxrsozcqb`) — и это НАМЕРЕННО. Чат интегрирован с конфигуратором: те же учётки
(`auth.users`/`profiles`), привязка чатов к заказам (`projects`/`orders`). Впереди десктоп/мобилка —
тоже на этих аккаунтах. Разносить по разным проектам НЕ надо — это убьёт интеграцию.

**Значит задача не «изолировать апки», а не дать разработке одной сломать ОБЩУЮ схему/RLS.** Guard —
это (а) правила для ассистента чата (`CRM Chat/chat-app/ПРАВИЛА…md`: читать общие таблицы можно,
менять их схему/RLS — нельзя), (б) точечный hardening `profiles` ниже (общая гигиена — служебные поля
профиля ставит только сервер), (в) `service_role`-ключ давать только доверенному серверному коду.

**Конкретные меры:**
1. `companies`/`orders`/`projects` уже под RLS с привязкой к `company_id` — аккаунт без компании/прав
   их не видит и не пишет. Чат-аккаунты (отдельные люди, не члены компании) уже отрезаны.
2. `profiles`: правки служебных полей (`company_id`, `is_admin`, `is_company_admin`, `is_active`)
   закрыты для клиента — их ставит только Edge Function конфигуратора на `service_role`. Это
   закрывает эскалацию (чат-юзер не сможет выдать себе доступ к конфигуратору через свой профиль).
3. `service_role`-ключ чат-приложению не давать (только anon).

**SQL зафиксирован:** `supabase/saas-05-shared-db-hardening.sql` — точечный `REVOKE` служебных колонок
`profiles` + диагностика + откат. ⚠️ Не применён к живой базе: сначала диагностика (п.0 в файле),
потом применить в SQL-редакторе по согласованию.
