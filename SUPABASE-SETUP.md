...
Supabase (`csntzolfgjlcxrsozcqb`). База общая. Таблица `profiles` — общая; рядом наши `companies`,
`orders`, `projects`.

**Честно про изоляцию:** обе апки ходят одним anon-ключом и ролью `authenticated` — база их не
различает. Поэтому «стены по приложению» на общем проекте технически не сделать. Настоящая изоляция —
это РАЗНЫЕ проекты Supabase (чат вынести на свой проект). Это рекомендуемое решение на будущее.

**Что реально защищает сейчас (без разделения проектов):**
